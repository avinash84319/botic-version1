...
app.post("/addproduct", function (req, res) {  //adding product
  ...
  customers.find({ phoneno: customer }, function (err, hash) {
    if (err) {
      ...
      for (var i = 0; i < hash[0].products.length; i++) {
        nid = hash[0].products[i].id + 1;         //getting no of products to add id
      }
      var ndate = new Date();
      console.log(ndate);
      var addproduct = {                          //object product adding to products array
        ...
        date: ndate.getDate() + "/" + ndate.getMonth() + "/" + ndate.getFullYear() //adding date to products array
      };
      customers.updateOne({ phoneno: customer }, {
        $push:  //pushing new object into prouct array
          { products: addproduct }
